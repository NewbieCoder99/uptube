...
class Uptube
{
    ...
    {
        if(
            !$this->app->config->get('uptube.client_id') ||
            !$this->app->config->get('uptube.client_secret')
        ) {
            throw new Exception('A Google "client_id" and "client_secret" must be configured.');
        }

        $client->setClientId($this->app->config->get('uptube.client_id'));
        $client->setClientSecret($this->app->config->get('uptube.client_secret'));
        $client->setScopes($this->app->config->get('uptube.scopes'));
        $client->setAccessType('offline');
        $client->setApprovalPrompt('force');
        $client->setRedirectUri(url(
            $this->app->config->get('uptube.routes.prefix')
            . '/' .
            $this->app->config->get('uptube.routes.redirect_uri')
        ));
